...
typedef struct {
    ...
    XFast1_core_uio_map maps[ MAX_UIO_MAPS ];
} XFast1_core_uio_info;

/***************** Variable Definitions **************************************/
static XFast1_core_uio_info uio_info;
...
static int uio_info_read_name(XFast1_core_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XFast1_core_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XFast1_core_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XFast1_core_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XFast1_core_Initialize(XFast1_core *InstancePtr, const char* InstanceName) {
	XFast1_core_uio_info *InfoPtr = &uio_info;
