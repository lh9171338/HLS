...
//------------------------Parameter----------------------
`define TV_IN_rows "./c.HoughLinesP_Core.autotvin_rows.dat"
`define TV_IN_cols "./c.HoughLinesP_Core.autotvin_cols.dat"
`define TV_IN_thresh "./c.HoughLinesP_Core.autotvin_thresh.dat"
`define TV_IN_lineGap "./c.HoughLinesP_Core.autotvin_lineGap.dat"
...
parameter lineGap_DEPTH = 1;
reg [31 : 0] lineGap_OPERATE_DEPTH = 0;
parameter lineGap_c_bitwidth = 32;
parameter START_ADDR = 0;
parameter HoughLinesP_Core_continue_addr = 0;
parameter HoughLinesP_Core_auto_start_addr = 0;
parameter rows_data_in_addr = 16;
parameter cols_data_in_addr = 24;
parameter thresh_data_in_addr = 32;
parameter lineGap_data_in_addr = 40;
...
reg [DATA_WIDTH - 1 : 0] mem_lineGap [lineGap_DEPTH - 1 : 0];
reg lineGap_write_data_finish;
...
//write lineGap reg
reg [31 : 0] write_lineGap_count = 0;
reg write_lineGap_run_flag = 0;
reg write_one_lineGap_data_done = 0;
...
assign TRAN_ctrl_write_data_finish = 1 & rows_write_data_finish & cols_write_data_finish & thresh_write_data_finish & lineGap_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        lineGap_write_data_finish <= 0;
        write_lineGap_run_flag <= 0; 
        write_lineGap_count = 0;
        count_operate_depth_by_bitwidth_and_depth (lineGap_c_bitwidth, lineGap_DEPTH, lineGap_OPERATE_DEPTH);
    end
    else begin
        if (TRAN_ctrl_start_in === 1) begin
            lineGap_write_data_finish <= 0;
        ...
        if (AESL_ready_reg === 1) begin
            write_lineGap_run_flag <= 1; 
            write_lineGap_count = 0;
        end
        if (write_one_lineGap_data_done === 1) begin
            write_lineGap_count = write_lineGap_count + 1;
            if (write_lineGap_count == lineGap_OPERATE_DEPTH) begin
                write_lineGap_run_flag <= 0; 
                lineGap_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_lineGap
    integer write_lineGap_resp;
    integer process_num ;
    integer get_ack;
    integer four_byte_num;
    integer c_bitwidth;
    integer i;
    integer j;
    reg [31 : 0] lineGap_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = lineGap_c_bitwidth;
    process_num = 4;
    count_c_data_four_byte_num_by_bitwidth (c_bitwidth , four_byte_num) ;
    while (1) begin
        process_4_finish <= 0;
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_lineGap_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write lineGap data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (lineGap_c_bitwidth < 32) begin
                        lineGap_data_tmp_reg = mem_lineGap[write_lineGap_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < lineGap_c_bitwidth) begin
                                lineGap_data_tmp_reg[j] = mem_lineGap[write_lineGap_count][i*32 + j];
                            end
                            else begin
                                lineGap_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    write (lineGap_data_in_addr + write_lineGap_count * four_byte_num * 4 + i * 4, lineGap_data_tmp_reg, write_lineGap_resp);
                end
                process_busy = 0;
                write_one_lineGap_data_done <= 1;
                @(posedge clk);
                write_one_lineGap_data_done <= 0;
            end   
            process_4_finish <= 1;
        end
        @(posedge clk);
...
// Read data from file 
initial begin : read_lineGap_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [127 : 0] token; 
  reg [127 : 0] token_tmp; 
  //reg [lineGap_c_bitwidth - 1 : 0] token_tmp; 
  reg [DATA_WIDTH - 1 : 0] mem_tmp; 
  reg [ 8*5 : 1] str;
  integer transaction_idx; 
  integer i; 
  transaction_idx = 0; 
  mem_tmp [DATA_WIDTH - 1 : 0] = 0;
  count_seperate_factor_by_bitwidth (lineGap_c_bitwidth , factor);
  fp = $fopen(`TV_IN_lineGap ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_lineGap); 
      $finish; 
  end 
  read_token(fp, token); 
  if (token != "[[[runtime]]]") begin             // Illegal format 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < lineGap_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ret = $sscanf(token, "0x%x", token_tmp); 
          if (factor == 4) begin
              if (i%factor == 0) begin
