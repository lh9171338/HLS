...
#ifndef _SIFT2_Core_dst1_val_H_
#define _SIFT2_Core_dst1_val_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "SIFT2_Core_dst1_val_memcore.h"

SC_MODULE(SIFT2_Core_dst1_val) {
...
protected:
    ...
    // buffer instances
    SIFT2_Core_dst1_val_memcore* buffer[BufferCount];
...
public:
    ~SIFT2_Core_dst1_val();
    SC_CTOR(SIFT2_Core_dst1_val) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "SIFT2_Core_dst1_val_memcore_U_";
            name += szTmp;

            buffer[i] = new SIFT2_Core_dst1_val_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "SIFT2_Core_dst1_val.trace");
            // ports
