...
#ifndef _HoughLines_Core_etde_H_
#define _HoughLines_Core_etde_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "HoughLines_Core_etde_memcore.h"

SC_MODULE(HoughLines_Core_etde) {
...
protected:
    ...
    HoughLines_Core_etde_memcore* HoughLines_Core_etde_memcore_U;
...
public:
    ...
    SC_CTOR(HoughLines_Core_etde) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        HoughLines_Core_etde_memcore_U = new HoughLines_Core_etde_memcore("HoughLines_Core_etde_memcore_U");
        HoughLines_Core_etde_memcore_U->clk      ( clk );
        HoughLines_Core_etde_memcore_U->reset    ( reset );
        HoughLines_Core_etde_memcore_U->ce0      ( i_ce0 );
        HoughLines_Core_etde_memcore_U->we0      ( i_we0 );
        HoughLines_Core_etde_memcore_U->address0 ( memcore_iaddr );
        HoughLines_Core_etde_memcore_U->d0       ( i_d0 );
        HoughLines_Core_etde_memcore_U->q0       ( i_q0 );

        HoughLines_Core_etde_memcore_U->ce1      ( t_ce0 );
        HoughLines_Core_etde_memcore_U->we1      ( t_we0 );
        HoughLines_Core_etde_memcore_U->address1 ( memcore_taddr );
        HoughLines_Core_etde_memcore_U->d1       ( t_d0 );
        HoughLines_Core_etde_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "HoughLines_Core_etde.trace");
            // ports
