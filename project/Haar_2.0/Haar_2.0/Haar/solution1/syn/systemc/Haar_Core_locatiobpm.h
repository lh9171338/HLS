...
#ifndef _Haar_Core_locatiobpm_H_
#define _Haar_Core_locatiobpm_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "Haar_Core_locatiobpm_memcore.h"

SC_MODULE(Haar_Core_locatiobpm) {
...
protected:
    ...
    // buffer instances
    Haar_Core_locatiobpm_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(Haar_Core_locatiobpm) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "Haar_Core_locatiobpm_memcore_U_";
            name += szTmp;

            buffer[i] = new Haar_Core_locatiobpm_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "Haar_Core_locatiobpm.trace");
            // ports
