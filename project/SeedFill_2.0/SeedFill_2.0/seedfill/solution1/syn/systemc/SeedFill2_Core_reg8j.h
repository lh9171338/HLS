...
#ifndef _SeedFill2_Core_reg8j_H_
#define _SeedFill2_Core_reg8j_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "SeedFill2_Core_reg8j_memcore.h"

SC_MODULE(SeedFill2_Core_reg8j) {
...
protected:
    ...
    // buffer instances
    SeedFill2_Core_reg8j_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(SeedFill2_Core_reg8j) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "SeedFill2_Core_reg8j_memcore_U_";
            name += szTmp;

            buffer[i] = new SeedFill2_Core_reg8j_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "SeedFill2_Core_reg8j.trace");
            // ports
