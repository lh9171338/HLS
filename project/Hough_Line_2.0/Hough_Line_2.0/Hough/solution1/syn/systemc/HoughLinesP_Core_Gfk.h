...
#ifndef _HoughLinesP_Core_Gfk_H_
#define _HoughLinesP_Core_Gfk_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "HoughLinesP_Core_Gfk_memcore.h"

SC_MODULE(HoughLinesP_Core_Gfk) {
...
protected:
    ...
    HoughLinesP_Core_Gfk_memcore* HoughLinesP_Core_Gfk_memcore_U;
...
public:
    ...
    SC_CTOR(HoughLinesP_Core_Gfk) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        HoughLinesP_Core_Gfk_memcore_U = new HoughLinesP_Core_Gfk_memcore("HoughLinesP_Core_Gfk_memcore_U");
        HoughLinesP_Core_Gfk_memcore_U->clk      ( clk );
        HoughLinesP_Core_Gfk_memcore_U->reset    ( reset );
        HoughLinesP_Core_Gfk_memcore_U->ce0      ( i_ce0 );
        HoughLinesP_Core_Gfk_memcore_U->we0      ( i_we0 );
        HoughLinesP_Core_Gfk_memcore_U->address0 ( memcore_iaddr );
        HoughLinesP_Core_Gfk_memcore_U->d0       ( i_d0 );
        HoughLinesP_Core_Gfk_memcore_U->q0       ( i_q0 );

        HoughLinesP_Core_Gfk_memcore_U->ce1      ( t_ce0 );
        HoughLinesP_Core_Gfk_memcore_U->we1      ( t_we0 );
        HoughLinesP_Core_Gfk_memcore_U->address1 ( memcore_taddr );
        HoughLinesP_Core_Gfk_memcore_U->d1       ( t_d0 );
        HoughLinesP_Core_Gfk_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "HoughLinesP_Core_Gfk.trace");
            // ports
