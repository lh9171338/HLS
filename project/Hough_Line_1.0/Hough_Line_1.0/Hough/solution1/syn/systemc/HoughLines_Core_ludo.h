...
#ifndef _HoughLines_Core_ludo_H_
#define _HoughLines_Core_ludo_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "HoughLines_Core_ludo_memcore.h"

SC_MODULE(HoughLines_Core_ludo) {
...
protected:
    ...
    // buffer instances
    HoughLines_Core_ludo_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(HoughLines_Core_ludo) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "HoughLines_Core_ludo_memcore_U_";
            name += szTmp;

            buffer[i] = new HoughLines_Core_ludo_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "HoughLines_Core_ludo.trace");
            // ports
