...
#ifndef _Haar_Core_src_val_H_
#define _Haar_Core_src_val_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "Haar_Core_src_val_memcore.h"

SC_MODULE(Haar_Core_src_val) {
...
protected:
    ...
    Haar_Core_src_val_memcore* Haar_Core_src_val_memcore_U;
...
public:
    ~Haar_Core_src_val();
    SC_CTOR(Haar_Core_src_val) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        Haar_Core_src_val_memcore_U = new Haar_Core_src_val_memcore("Haar_Core_src_val_memcore_U");
        Haar_Core_src_val_memcore_U->clk      ( clk );
        Haar_Core_src_val_memcore_U->reset    ( reset );
        Haar_Core_src_val_memcore_U->ce0      ( i_ce0 );
        Haar_Core_src_val_memcore_U->we0      ( i_we0 );
        Haar_Core_src_val_memcore_U->address0 ( memcore_iaddr );
        Haar_Core_src_val_memcore_U->d0       ( i_d0 );
        Haar_Core_src_val_memcore_U->q0       ( i_q0 );

        Haar_Core_src_val_memcore_U->ce1      ( t_ce0 );
        Haar_Core_src_val_memcore_U->we1      ( t_we0 );
        Haar_Core_src_val_memcore_U->address1 ( memcore_taddr );
        Haar_Core_src_val_memcore_U->d1       ( t_d0 );
        Haar_Core_src_val_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "Haar_Core_src_val.trace");
            // ports
