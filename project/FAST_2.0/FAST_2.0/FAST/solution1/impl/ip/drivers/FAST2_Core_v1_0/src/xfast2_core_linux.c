...
typedef struct {
    ...
    XFast2_core_uio_map maps[ MAX_UIO_MAPS ];
} XFast2_core_uio_info;

/***************** Variable Definitions **************************************/
static XFast2_core_uio_info uio_info;
...
static int uio_info_read_name(XFast2_core_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XFast2_core_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XFast2_core_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XFast2_core_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XFast2_core_Initialize(XFast2_core *InstancePtr, const char* InstanceName) {
	XFast2_core_uio_info *InfoPtr = &uio_info;
