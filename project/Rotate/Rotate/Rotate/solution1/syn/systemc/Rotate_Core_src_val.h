...
#ifndef _Rotate_Core_src_val_H_
#define _Rotate_Core_src_val_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "Rotate_Core_src_val_memcore.h"

SC_MODULE(Rotate_Core_src_val) {
...
protected:
    ...
    // buffer instances
    Rotate_Core_src_val_memcore* buffer[BufferCount];
...
public:
    ~Rotate_Core_src_val();
    SC_CTOR(Rotate_Core_src_val) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "Rotate_Core_src_val_memcore_U_";
            name += szTmp;

            buffer[i] = new Rotate_Core_src_val_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "Rotate_Core_src_val.trace");
            // ports
