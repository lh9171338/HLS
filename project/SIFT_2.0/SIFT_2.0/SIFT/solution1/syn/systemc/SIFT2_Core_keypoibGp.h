...
#ifndef _SIFT2_Core_keypoibGp_H_
#define _SIFT2_Core_keypoibGp_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "SIFT2_Core_keypoibGp_memcore.h"

SC_MODULE(SIFT2_Core_keypoibGp) {
...
protected:
    ...
    SIFT2_Core_keypoibGp_memcore* SIFT2_Core_keypoibGp_memcore_U;
...
public:
    ...
    SC_CTOR(SIFT2_Core_keypoibGp) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        SIFT2_Core_keypoibGp_memcore_U = new SIFT2_Core_keypoibGp_memcore("SIFT2_Core_keypoibGp_memcore_U");
        SIFT2_Core_keypoibGp_memcore_U->clk      ( clk );
        SIFT2_Core_keypoibGp_memcore_U->reset    ( reset );
        SIFT2_Core_keypoibGp_memcore_U->ce0      ( i_ce0 );
        SIFT2_Core_keypoibGp_memcore_U->we0      ( i_we0 );
        SIFT2_Core_keypoibGp_memcore_U->address0 ( memcore_iaddr );
        SIFT2_Core_keypoibGp_memcore_U->d0       ( i_d0 );
        SIFT2_Core_keypoibGp_memcore_U->q0       ( i_q0 );

        SIFT2_Core_keypoibGp_memcore_U->ce1      ( t_ce0 );
        SIFT2_Core_keypoibGp_memcore_U->we1      ( t_we0 );
        SIFT2_Core_keypoibGp_memcore_U->address1 ( memcore_taddr );
        SIFT2_Core_keypoibGp_memcore_U->d1       ( t_d0 );
        SIFT2_Core_keypoibGp_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "SIFT2_Core_keypoibGp.trace");
            // ports
