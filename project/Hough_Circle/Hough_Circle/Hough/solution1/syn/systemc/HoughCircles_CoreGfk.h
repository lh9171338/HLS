...
#ifndef _HoughCircles_CoreGfk_H_
#define _HoughCircles_CoreGfk_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "HoughCircles_CoreGfk_memcore.h"

SC_MODULE(HoughCircles_CoreGfk) {
...
protected:
    ...
    // buffer instances
    HoughCircles_CoreGfk_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(HoughCircles_CoreGfk) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "HoughCircles_CoreGfk_memcore_U_";
            name += szTmp;

            buffer[i] = new HoughCircles_CoreGfk_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "HoughCircles_CoreGfk.trace");
            // ports
