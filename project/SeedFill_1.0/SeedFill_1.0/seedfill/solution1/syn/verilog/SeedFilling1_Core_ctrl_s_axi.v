...
`timescale 1ns/1ps
module SeedFilling1_Core_ctrl_s_axi
#(parameter
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (num_ap_vld)
            int_num_ap_vld <= 1'b1;
        else if (ar_hs && raddr == ADDR_NUM_CTRL)
            int_num_ap_vld <= 1'b0; // clear on read
    end
end
