...
entity Haar_Core_ctrl_s_axi is
generic (
...
end entity Haar_Core_ctrl_s_axi;
...
architecture behave of Haar_Core_ctrl_s_axi is
    ...
    constant ADDR_SCALE_DATA_0     : INTEGER := 16#20#;
    constant ADDR_SCALE_CTRL       : INTEGER := 16#24#;
    constant ADDR_NEIGHBORS_DATA_0 : INTEGER := 16#28#;
    constant ADDR_NEIGHBORS_CTRL   : INTEGER := 16#2c#;
    ...
    signal int_neighbors       : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_NEIGHBORS_DATA_0 =>
                        rdata_data <= RESIZE(int_neighbors(31 downto 0), 32);
                    when others =>
                        rdata_data <= (others => '0');
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    neighbors            <= STD_LOGIC_VECTOR(int_neighbors);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_NEIGHBORS_DATA_0) then
                    int_neighbors(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_neighbors(31 downto 0));
                end if;
            end if;
        end if;
    end process;
