...
#ifndef _HoughLinesP_Core_Hfu_H_
#define _HoughLinesP_Core_Hfu_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "HoughLinesP_Core_Hfu_memcore.h"

SC_MODULE(HoughLinesP_Core_Hfu) {
...
protected:
    ...
    // buffer instances
    HoughLinesP_Core_Hfu_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(HoughLinesP_Core_Hfu) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "HoughLinesP_Core_Hfu_memcore_U_";
            name += szTmp;

            buffer[i] = new HoughLinesP_Core_Hfu_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "HoughLinesP_Core_Hfu.trace");
            // ports
