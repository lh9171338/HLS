...
Haar_Core_locatiobpm::~Haar_Core_locatiobpm() {
...
void Haar_Core_locatiobpm::proc_i_full_n() {
    i_full_n.write(full_n.read());
}

void Haar_Core_locatiobpm::proc_t_empty_n() {
    t_empty_n.write(empty_n.read());
}

void Haar_Core_locatiobpm::proc_i_q0() {
    i_q0.write(buf_q0[iptr.read()].read());
}

void Haar_Core_locatiobpm::proc_t_q0() {
    t_q0.write(buf_q0[tptr.read()].read());
}

void Haar_Core_locatiobpm::proc_buffer_signals() {
    for (unsigned i = 0; i < BufferCount; i++) {
        if (iptr.read() == i) {
...
void Haar_Core_locatiobpm::proc_push_buf() {
    push_buf.write(i_ce.read() & i_write.read() & full_n.read());
}

void Haar_Core_locatiobpm::proc_pop_buf() {
    pop_buf.write(t_ce.read() & t_read.read() & empty_n.read());
}

void Haar_Core_locatiobpm::proc_iptr() {
...
void Haar_Core_locatiobpm::proc_tptr() {
    if (reset.read() == SC_LOGIC_1) {
        tptr.write(0);
...
void Haar_Core_locatiobpm::proc_count() {
    if (reset.read() == SC_LOGIC_1) {
        count.write(0);
...
void Haar_Core_locatiobpm::proc_full_n() {
    if (reset.read() == SC_LOGIC_1) {
...
void Haar_Core_locatiobpm::proc_empty_n() {
    if (reset.read() == SC_LOGIC_1) {
