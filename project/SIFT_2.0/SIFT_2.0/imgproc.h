...
void Filter2D(
	...
	Calculation					calculation = correlation
	)
{
	int rows = src.rows;
	int cols = src.cols;
...
#pragma HLS PIPELINE II=1
			...
			if(i >= KH-1 && j >= KW-1){
				...
				if(Round){
					dst.val[i-KH+1][j-KW+1] = sum+(K_T)0.5; // 四舍五入
				}
				else{
	...
	)
{
    ...
    Filter2D(src, kernel, dst, Round, replicate);
}
	...
	)
{
	int rows = src.rows;
	int cols = src.cols;
	int _rows = round(rows*scale);
	int _cols = round(cols*scale);
	assert(_rows <= DROWS);
	assert(_cols <= DCOLS);
	dst.rows = _rows;
	dst.cols = _cols;

	fix32_16 iscale = (fix32_16)(1.0f/scale);
	fix32_16 r;
	fix32_16 c;
	int r0;
	int c0;
...
#pragma HLS PIPELINE II=2
	        	...
	        	if(Round){
	        		dst.val[i][j] = sum+(fix32_16)0.5; // 四舍五入
	        	}
	        	else{
	        		dst.val[i][j] = sum;
	        	}
