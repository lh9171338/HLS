...
#ifndef _HoughCircles_CoreFfa_H_
#define _HoughCircles_CoreFfa_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "HoughCircles_CoreFfa_memcore.h"

SC_MODULE(HoughCircles_CoreFfa) {
...
protected:
    ...
    HoughCircles_CoreFfa_memcore* HoughCircles_CoreFfa_memcore_U;
...
public:
    ...
    SC_CTOR(HoughCircles_CoreFfa) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        HoughCircles_CoreFfa_memcore_U = new HoughCircles_CoreFfa_memcore("HoughCircles_CoreFfa_memcore_U");
        HoughCircles_CoreFfa_memcore_U->clk      ( clk );
        HoughCircles_CoreFfa_memcore_U->reset    ( reset );
        HoughCircles_CoreFfa_memcore_U->ce0      ( i_ce0 );
        HoughCircles_CoreFfa_memcore_U->we0      ( i_we0 );
        HoughCircles_CoreFfa_memcore_U->address0 ( memcore_iaddr );
        HoughCircles_CoreFfa_memcore_U->d0       ( i_d0 );
        HoughCircles_CoreFfa_memcore_U->q0       ( i_q0 );

        HoughCircles_CoreFfa_memcore_U->ce1      ( t_ce0 );
        HoughCircles_CoreFfa_memcore_U->we1      ( t_we0 );
        HoughCircles_CoreFfa_memcore_U->address1 ( memcore_taddr );
        HoughCircles_CoreFfa_memcore_U->d1       ( t_d0 );
        HoughCircles_CoreFfa_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "HoughCircles_CoreFfa.trace");
            // ports
