...
localparam
    ...
    ADDR_SCALE_DATA_0     = 6'h20,
    ADDR_SCALE_CTRL       = 6'h24,
    ADDR_NEIGHBORS_DATA_0 = 6'h28,
    ADDR_NEIGHBORS_CTRL   = 6'h2c,
    WRIDLE                = 2'd0,
    WRDATA                = 2'd1,
    WRRESP                = 2'd2,
    WRRESET               = 2'd3,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_NEIGHBORS_DATA_0: begin
                    rdata <= int_neighbors[31:0];
                end
            endcase
        end
    end
...
assign neighbors = int_neighbors;
// int_ap_start
always @(posedge ACLK) begin
    if (ARESET)
        int_ap_start <= 1'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        int_neighbors[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_NEIGHBORS_DATA_0)
            int_neighbors[31:0] <= (WDATA[31:0] & wmask) | (int_neighbors[31:0] & ~wmask);
    end
end
